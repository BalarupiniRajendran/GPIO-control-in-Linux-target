...
int time_calc_for_usleep(int dc, int on){ /*calculates the sleep duration based on given duty cycle*/
  int temp;
  if(on ==1)
    temp = dc/100;
  else
...
int* set_pin_values(int pin, int *npin) // assigning the values for gpio pins based on pin number
{
